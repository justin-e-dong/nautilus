...
{
    nk_thread_t * curr= get_cur_thread;
    nk_thread_t * next = _sched_need_resched(0,0);
    if (!next){
        goto out;
    }
    // This is where would we put timers for the interrupt thread timing //
    if (curr->sched_state->is_intr && !next->sched_state->is_intr){
        // now we know we are swithcing out of the interrupt thread
        //record time
    }
    if (!curr->sched_state->is_intr && next->sched_state->is_intr){
        // now we know we are switching into the interrupt thead
